...
function initMobileMenu() {
  ...
  function toggleMenu() {
    ...
    menuToggle.setAttribute(
      "aria-label",
      isActive ? "Close mobile menu" : "Open mobile menu",
    );
  ...
  if (form) {
    ...
    if (csrfField) {
      fetch("csrf-token.php")
        .then((r) => r.json())
        .then((d) => {
          if (d && d.token) csrfField.value = d.token;
        })
        .catch(() => {});
    }
  ...
  if (prefersReducedMotion()) {
    statNumbers.forEach((stat) => {
      const target = parseInt(
        stat.getAttribute("data-count") || stat.textContent,
        10,
      );
      stat.textContent = Number.isFinite(target)
